...
class PingMsgHandler(MsgHandler):
    ...
    def likesUs(self):
        return self.answers and not self.rejects
    def isAlive(self):
        return self.answers
    def processMsg(self, msg):
        self.rejects = msg.isNackOfDirect()
        self.answers = True
        return 1
...
class Device:
    ...
    def getdb(self):
        ...
        self.dbbuilder.wait()
        self.dbbuilder.db.dump()
    def readdb(self):
        """readdb()
        blocking wait for the device database and return it"""
        iofun.out("reading device db, be patient!")
        self.dbbuilder.clear()
        self.dbbuilder.start()
        isSane = self.dbbuilder.wait()
        return self.dbbuilder.db, isSane
...
#
    ...
    def ping(self):
        ...
        pings device"""
        pmh = PingMsgHandler()
        self.querier.setMsgHandler(pmh)
        self.querier.querysd(0x0F, 0x01);
        gotReply = self.querier.waitForReply(5.0)
        if gotReply:
            if pmh.likesUs():
                print "got ack!"
                return 1
